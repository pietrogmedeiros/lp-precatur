...
import { isMountPoint } from "./volume.js";
...
const dataVolume = isMountPoint(config.dataDir);
const app = createApp({ config, store, delivery, dataVolume });
...
const server = app.listen(config.port, config.host, () => {
  ...
  if (dataVolume === false) {
    console.warn(`[server] ATENÇÃO: ${config.dataDir} não é um volume; os leads serão apagados no próximo deploy.`);
  }
